from sqlalchemy import Integer, Numeric, String, VARCHAR, SmallInteger, ForeignKey, func
...
class Kiosk(Base):
    ...
    descr: Mapped[Optional[str]] = mapped_column("DESCR", VARCHAR(100))
    creation_date: Mapped[Optional[datetime]] = mapped_column("CREATION_DATE",
                                                              server_default=func.current_timestamp())
    updated_date: Mapped[Optional[datetime]] = mapped_column("UPDATED_DATE",
    ...
    commission_check: Mapped[Optional[str]] = mapped_column("COMMISSION_CHECK", String(1))
    service_group_check: Mapped[Optional[str]] = mapped_column("SERVICE_GROUP_CHECK", String(1))
    service_charge_check: Mapped[Optional[str]] = mapped_column("SERVICE_CHARGE_CHECK", String(1))
    fk_service_grouno: Mapped[Optional[int]] = mapped_column("FK_SERVICE_GROUNO", SmallInteger,
                                                             ForeignKey("SERVICE_GROUP.NO"), index=True)
    fk_addressid: Mapped[Optional[int]] = mapped_column("FK_ADDRESSID", SmallInteger)
    fk_kiosk_familyid: Mapped[Optional[int]] = mapped_column("FK_KIOSK_FAMILYID", Integer,
                                                             ForeignKey("KIOSK_FAMILY.ID"), index=True)
    fk_commession_gcd: Mapped[Optional[int]] = mapped_column("FK_COMMESSION_GCD", Integer)
...
class KioskFamily(Base):
    ...
    deleted_flag: Mapped[Optional[int]] = mapped_column("DELETED_FLAG", SmallInteger)
    fk_commession_gcd: Mapped[Optional[int]] = mapped_column("FK_COMMESSION_GCD", Integer)
    fk_service_charcd: Mapped[Optional[int]] = mapped_column("FK_SERVICE_CHARCD", Integer)
    fk_service_grouno: Mapped[Optional[int]] = mapped_column("FK_SERVICE_GROUNO", SmallInteger)
    # fk_kiosk_familyid: Mapped[Optional[int]] = mapped_column("FK_KIOSK_FAMILYID", Integer)
...
class KioskOperatorLog(Base):
    ...
    ip_address: Mapped[Optional[str]] = mapped_column("IP_ADDRESS", VARCHAR(15))
    entrystamp: Mapped[Optional[datetime]] = mapped_column("ENTRYSTAMP",
                                                           server_default=func.current_timestamp())
    ...
    type: Mapped[Optional[str]] = mapped_column("TYPE", String(1))
    fk_kioskid: Mapped[Optional[int]] = mapped_column("FK_KIOSKID", Integer, ForeignKey("KIOSK.ID"))
    fk_transaction_number: Mapped[Optional[int]] = mapped_column("FK_TRANSACTION_NUMBER", Integer)
