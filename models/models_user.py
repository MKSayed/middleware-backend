...
from sqlalchemy import (String, SmallInteger, Date, Time, PrimaryKeyConstraint,
...
class User(Base):
    ...
    username: Mapped[str] = mapped_column("USERNAME", String(25), unique=True, nullable=False)
    ...
    fk_user_typecd: Mapped[int] = mapped_column("FK_USER_TYPECD", ForeignKey("USER_TYPE.CD"),
...
class UserLog(Base):
    ...
    fk_userid: Mapped[int] = mapped_column("FK_USERID", ForeignKey("USER.ID"), nullable=False, index=True)
...
class UserLogHistory(Base):
    ...
    fk_user_logdate: Mapped[date] = mapped_column("FK_USER_LOGDATE", Date, nullable=False, index=True)
    fk_user_logtime: Mapped[time] = mapped_column("FK_USER_LOGTIME", Time, nullable=False, index=True)
    fk_user_logfk_userid: Mapped[int] = mapped_column("FK_USER_LOGFK_USERID", nullable=False, index=True)
