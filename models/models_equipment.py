...
class AssignedEquipment(Base):
    ...
    fk_kioskid: Mapped[int] = mapped_column("FK_KIOSKID", ForeignKey('KIOSK.ID'), nullable=False, index=True)
    fk_kiosk_equipmid: Mapped[int] = mapped_column("FK_KIOSK_EQUIPMID",
                                                   ForeignKey("KIOSK_EQUIPMENT.ID"), nullable=False, index=True)

    __table_args__ = (PrimaryKeyConstraint('TIMESTAMP', 'FK_KIOSKID', 'FK_KIOSK_EQUIPMID'),)
...
class KioskEquipment(Base):
    ...
    component_ser_num: Mapped[Optional[str]] = mapped_column("COMPONENT_SER_NUM", String(20))
    component_ip_address: Mapped[Optional[str]] = mapped_column("COMPONENT_IP_ADDRESS", String(15))
    ...
    end_date: Mapped[Optional[date]] = mapped_column("END_DATE")
    fk_equipment_tycd: Mapped[Optional[int]] = mapped_column("FK_EQUIPMENT_TYCD",
                                                             ForeignKey("EQUIPMENT_TYPE.CD"), index=True)
