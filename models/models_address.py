...
class Address(Base):
    ...
    name: Mapped[str] = mapped_column(
        "NAME", String(35), nullable=False, comment="abbreviation for the place"
    )
    details: Mapped[Optional[str]] = mapped_column("DETAILS", String(65))
    fk_police_staticd: Mapped[Optional[int]] = mapped_column(
        "FK_POLICE_STATICD", ForeignKey("POLICE_STATION.CD"), index=True
    )
    fk_areacd: Mapped[Optional[int]] = mapped_column(
        "FK_AREACD", ForeignKey("AREA.CD"), index=True
    )
    fk_address_typecd: Mapped[Optional[int]] = mapped_column(
        "FK_ADDRESS_TYPECD", ForeignKey("ADDRESS_TYPE.CD"), index=True
    )
